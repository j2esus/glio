...
public class CategoryController extends BaseController{
    ...
    public List<Category> findCategories(HttpServletRequest request, @RequestParam String name){
        return categoryService.findBy(getCurrentCompany(request), new Status[]{Status.ACTIVE, Status.INACTIVE}, name);
    }
    ...
    public String saveCategory(HttpServletRequest request, @RequestParam Integer id, @RequestParam String name,@RequestParam Status status){
        try{
            categoryService.saveOrUpdate(new Category(id.equals(0) ? null : id, name, status, getCurrentCompany(request)));
            return "OK";
        }catch(Exception e){
            return e.getMessage();
        }
    ...
    public String deleteCategory(HttpServletRequest request,@RequestParam Integer id){
        try{
            this.categoryService.changeStatus(categoryService.findBy(id), Status.DELETED);
            return "OK";
        }catch(Exception e){
            return e.getMessage();
        }
    ...
    public List<Subcategory> findSubcategories(HttpServletRequest request, @RequestParam Integer idCategory, @RequestParam String name){
        ...
        return subcategoryService.findBy(category, new Status[]{Status.ACTIVE,Status.INACTIVE}, name);
    }
    ...
    public String saveSubcategory(HttpServletRequest request, @RequestParam Integer id, @RequestParam String name,@RequestParam Status status,
        ...
        try{
            subcategoryService.saveOrUpdate(new Subcategory(id.equals(0) ? null : id, name, status, categoryService.findBy(idCategory)));
            return "OK";
        }catch(Exception e){
            return e.getMessage();
        }
    ...
    public String deleteSubcategory(HttpServletRequest request,@RequestParam Integer id){
        try{
            this.subcategoryService.changeStatus(subcategoryService.findBy(id), Status.DELETED);
            return "OK";
        }catch(Exception e){
            return e.getMessage();
        }

...
public class StockOutController extends BaseController {
    ...
    @Autowired
    public StockOutController(DepotService depotService, ArticleService articleService,
    ...
    public String index(HttpServletRequest request, Model model){
        ...
        model.addAttribute("title", "Salidas");
        model.addAttribute("depots", depots);
        model.addAttribute("stockType", "stockOut");
        return "stockIn/init";
    }
    ...
    public ResponseEntity executeMovement(HttpServletRequest request, @RequestParam Integer idArticle,
                                   @RequestParam Integer idDepot, @RequestParam Integer quantity,
                                   @RequestParam String description){
        stockService.take(new Stock(Util.getCurrentDate(), getCurrentUser(request), depotService.findById(idDepot),
                articleService.findBydId(idArticle), quantity, description, StockType.OUT, getCurrentCompany(request)));

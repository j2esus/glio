...
public class TaskDAOTest {
    ...
    private final static User admin = new User(1, "admin@company", "password", "admin", Status.ACTIVE,
            ...
            false, new Company(1, "company", "description", Status.ACTIVE, 3), "admin@company.com");
    private static final User worker = new User(2, "worker@company", "password", "worker", Status.ACTIVE,
            ...
            false, new Company(1, "company", "description", Status.ACTIVE, 3), "worker@company.com");
    ...
    private static List<Task> tasksExpected = new ArrayList<>();
    static {
        tasksExpected.add(new Task(1, "screen", "create screen", Status.ACTIVE, Priority.MEDIA, 8, admin, worker, expectedAim));
        tasksExpected.add(new Task(3, "service", "create service", Status.ACTIVE, Priority.BAJA, 8, admin, worker, expectedAim));
        tasksExpected.add(new Task(4, "dao", "create dao", Status.ACTIVE, Priority.ALTA, 8, admin, worker, expectedAim));
    }
    ...
    @Test
    public void findByProject_project_listWithThreeElements(){
        assertThat(taskDAO.findByProject(project)).isEqualTo(tasksExpected);
    }
    ...
    private void insertInitialData() throws SQLException {
        ...
        connection.createStatement().execute("insert into task(id_task, name, description, status, priority, estimated_time, id_aim, id_user_requester, id_user_owner)"+
                " values(1, 'screen', 'create screen', 'ACTIVE', 1, 8, 1, 1, 2)");
